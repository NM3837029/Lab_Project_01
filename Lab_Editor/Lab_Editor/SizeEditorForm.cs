...
// 敵（等）の表示スケールをドラッグ操作で視覚的に調整するダイアログ。
// HitboxEditorForm と同様のドラッグUIパターンを踏襲する。
public class SizeEditorForm : Form
{
    ...
    public SizeEditorForm(string imagePath, float initialScale)
    {
        ...
        btnSave = new Button { Text = "保存", Location = new Point(400, 512), Size = new Size(80, 32), BackColor = Color.FromArgb(40, 167, 69), ForeColor = Color.White, FlatStyle = FlatStyle.Flat };
        btnSave.Click += (s, e) => { ResultScale = currentScale; DialogResult = DialogResult.OK; Close(); };
